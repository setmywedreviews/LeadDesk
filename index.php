...
if($_POST['action']==='add_lead'&&$isAdmin){$business=trim($_POST['business_name']??'');$category=trim($_POST['category']??'');$city=trim($_POST['city']??'');$phone=trim($_POST['phone']??'');$instagram=trim($_POST['instagram']??'');$website=trim($_POST['website']??'');$assigned=(int)($_POST['assigned_to']??0);$score=(int)($_POST['lead_score']??50);if($business&&in_array($category,['Photography','Makeup Artist'],true)&&$assigned>0){try{$x=$pdo->prepare('INSERT INTO leads(business_name,category,city,phone,instagram,website,source,lead_score,assigned_to) VALUES(?,?,?,?,?,?,?,?,?)');$x->execute([$business,$category,$city,$phone?:null,$instagram?:null,$website?:null,'Manual',$score,$assigned]);}catch(Throwable $e){}}header('Location:?view=admin');exit;}$id=(int)($_POST['id']??0);$allowed=$isAdmin||($id>0&&$pdo->query("SELECT assigned_to FROM leads WHERE id=$id")->fetchColumn()==$me);if($allowed){if($_POST['action']==='remark'){$r=trim($_POST['remark']??'');$pdo->prepare('UPDATE leads SET remark=?,updated_at=NOW() WHERE id=?')->execute([$r,$id]);$pdo->prepare('INSERT INTO activities(lead_id,user_id,type,note) VALUES(?,?,?,?)')->execute([$id,$me,'remark',$r]);}if($_POST['action']==='followup'){$dt=$_POST['next']??'';$pdo->prepare("UPDATE leads SET next_followup=?,status='Follow-up',updated_at=NOW() WHERE id=?")->execute([$dt,$id]);$pdo->prepare('INSERT INTO activities(lead_id,user_id,type,note) VALUES(?,?,?,?)')->execute([$id,$me,'followup','Follow-up scheduled for '.$dt]);}if($_POST['action']==='status'){$st=$_POST['status'];$pdo->prepare('UPDATE leads SET status=?,updated_at=NOW() WHERE id=?')->execute([$st,$id]);$pdo->prepare('INSERT INTO activities(lead_id,user_id,type,note) VALUES(?,?,?,?)')->execute([$id,$me,'status',$st]);}}header('Location:'.($_SERVER['HTTP_REFERER']??'/'));exit;}
...
<div id="notifyBanner" class="card" style="display:none;margin-top:15px;background:#eff6ff"><b>🔔 Enable notifications</b><div class="muted" style="margin:5px 0 10px">Allow LeadDesk to notify you when your daily leads are ready.</div><button id="enableNotify" class="btn primary">Enable Notifications</button></div><div class="nav"><a class="btn <?= $view==='dashboard'?'primary':''?>" href="/">⌂ Dashboard</a><a class="btn <?= $view==='leads'?'primary':''?>" href="?view=leads">📋 My Leads</a><a class="btn <?= $view==='new'?'primary':''?>" href="?view=new&cat=<?=urlencode($cat)?>">🆕 New Leads</a><a class="btn <?= $view==='followups'?'primary':''?>" href="?view=followups">⏰ Follow-ups</a><a class="btn <?= $view==='starred'?'primary':''?>" href="?view=starred">⭐ Starred</a><?php if($isAdmin):?><a class="btn" href="?view=admin">⚙️ Admin</a><?php endif;?><a class="btn <?= $view==='daily_report'?'primary':''?>" href="?view=daily_report">📷 Daily Report</a><?php if($isAdmin):?><a class="btn <?= $view==='reports'?'primary':''?>" href="?view=reports">📊 Reports</a><?php endif;?></div>
...
const jokes=[
 "पत्नी: सुनो, अगर मैं दो दिन के लिए मायके चली जाऊँ तो तुम्हें कैसा लगेगा?\nपति: बहुत अच्छा।\nपत्नी: और अगर दो महीने के लिए?\nपति: बहुत-बहुत अच्छा।\nपत्नी: और अगर हमेशा के लिए?\nपति: अब इतना अच्छा भी नहीं लगना चाहिए। 😂",
 "दोस्त: भाई, तेरी शादी को कितने साल हो गए?\nदूसरा: पाँच साल।\nदोस्त: फिर भी खुश कैसे रहता है?\nदूसरा: मैं बोलता ही नहीं।\nदोस्त: घर में?\nदूसरा: नहीं, किसी को भी अपनी शादी की सलाह नहीं देता। 😂",
 "पत्नी: तुम मेरी कोई बात मानते क्यों नहीं हो?\nपति: मानता हूँ ना।\nपत्नी: कब?\nपति: जब तुम कहती हो 'जो करना है करो'… तब मैं कुछ नहीं करता। 😂",
 "पापा: बेटा, रिजल्ट कैसा आया?\nबेटा: पापा, एक अच्छी खबर है और एक बुरी।\nपापा: पहले अच्छी बता।\nबेटा: इस बार आपका गुस्सा पिछली बार से कम होगा। 😂",
 "टीचर: अगर तुम्हारे पास 10 आम हैं और मैं 2 ले लूँ, तो तुम्हारे पास क्या बचेगा?\nछात्र: सर, दुश्मनी। 😂",
 "दोस्त: भाई, तू इतना लेट क्यों आया?\nदूसरा: रास्ते में बोर्ड लगा था— 'आगे स्कूल है, धीरे चलें।'\nदोस्त: तो?\nदूसरा: मैं धीरे-धीरे आया… 2 घंटे लग गए। 😂",
 "मम्मी: बेटा, मेहमान आने वाले हैं, कमरे की सफाई कर दे।\nबेटा: मम्मी, मेहमान कितने दिन रुकेंगे?\nमम्मी: क्यों?\nबेटा: उसी हिसाब से सफाई की मेहनत तय करूंगा। 😂",
 "दोस्त: भाई, तूने नई घड़ी क्यों खरीदी?\nदूसरा: पुरानी में समय खराब था।\nदोस्त: घड़ी में?\nदूसरा: नहीं, मेरी जिंदगी में। 😂",
 "पत्नी: आज खाने में क्या बनाऊँ?\nपति: जो तुम्हारा मन करे।\nपत्नी: ठीक है।\nपति: लेकिन मेरा मन पूछ लेतीं तो अच्छा होता। 😂",
 "एक आदमी डॉक्टर के पास गया।\nआदमी: डॉक्टर साहब, मुझे भूलने की बीमारी है।\nडॉक्टर: कब से?\nआदमी: क्या कब से? 😂",
 "दोस्त: भाई, तू इतना शांत कैसे रहता है?\nदूसरा: जब भी गुस्सा आता है, मैं 10 तक गिनता हूँ।\nदोस्त: फिर?\nदूसरा: फिर सामने वाला 10 तक गिनने का मौका ही नहीं देता। 😂",
