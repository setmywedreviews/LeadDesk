...
try{$pdo->exec("ALTER TABLE leads MODIFY COLUMN status ENUM('New','Follow-up','Ringing','Interested','Not Interested','Converted','Wrong Number','DNC') DEFAULT 'New'");}catch(Throwable $e){}
...
$mf0=$pdo->query("SELECT m.*,u.category FROM manual_followups m LEFT JOIN users u ON u.id=m.user_id ORDER BY m.id");
foreach($mf0->fetchAll() as $m0){
 $mc=in_array($m0['category'],['Photography','Makeup Artist'],true)?$m0['category']:'Photography';
 $phone0=trim($m0['phone']??''); $existingId=0;
 if($phone0){$qq=$pdo->prepare('SELECT id FROM leads WHERE phone=? AND category=? LIMIT 1');$qq->execute([$phone0,$mc]);$existingId=(int)($qq->fetchColumn()?:0);}
 if(!$existingId){$qq=$pdo->prepare('SELECT id FROM leads WHERE business_name=? AND assigned_to=? AND category=? LIMIT 1');$qq->execute([$m0['business_name'],$m0['user_id'],$mc]);$existingId=(int)($qq->fetchColumn()?:0);}
 if($existingId){
  $pdo->prepare("UPDATE leads SET status='Follow-up',next_followup=?,remark=?,source=?,assigned_to=?,existing_customer=0,updated_at=NOW() WHERE id=?")->execute([$m0['followup_at'],$m0['remark'],$m0['lead_source']?:'Other',$m0['user_id'],$existingId]);
 }else{
  $pdo->prepare("INSERT INTO leads(business_name,category,phone,source,status,assigned_to,remark,next_followup) VALUES(?,?,?,?, 'Follow-up',?,?,?)")->execute([$m0['business_name'],$mc,$phone0?:null,$m0['lead_source']?:'Other',$m0['user_id'],$m0['remark'],$m0['followup_at']]);
 }
 $pdo->prepare('DELETE FROM manual_followups WHERE id=?')->execute([$m0['id']]);
}
...
$users=$pdo->query("SELECT id,name,category FROM users WHERE role='sales' AND active=1 ORDER BY id")->fetchAll();$where=[];$p=[];if(!$isAdmin){$where[]='l.assigned_to=?';$p[]=$me;}if(isset($_GET['batch_start'])&&$_GET['batch_start']!==''){ $where[]='l.created_at>=?'; $p[]=$_GET['batch_start']; }if(isset($_GET['batch_end'])&&$_GET['batch_end']!==''){ $where[]='l.created_at<=?'; $p[]=$_GET['batch_end']; }if($q){$where[]='(l.business_name LIKE ? OR l.city LIKE ? OR l.phone LIKE ?)';array_push($p,"%$q%","%$q%","%$q%");}if($cat){$where[]='l.category=?';$p[]=$cat;}if($isAdmin&&$emp){$where[]='l.assigned_to=?';$p[]=$emp;}if($view==='existing')$where[]='l.existing_customer=1';else{$where[]='l.existing_customer=0';if($view==='new')$where[]="l.status='New'";if($view==='interested')$where[]="l.status='Interested'";if($view==='ringing')$where[]="l.status='Ringing'";if($view==='followups')$where[]="(l.status='Follow-up' OR l.next_followup IS NOT NULL)";if($view==='starred')$where[]='l.starred=1';}if($dateFrom){$where[]='DATE(l.created_at)>=?';$p[]=$dateFrom;}if($dateTo){$where[]='DATE(l.created_at)<=?';$p[]=$dateTo;}
$sql="SELECT l.*,u.name assigned_name FROM leads l LEFT JOIN users u ON u.id=l.assigned_to".($where?' WHERE '.implode(' AND ',$where):'')." ORDER BY CASE WHEN l.starred=1 THEN 0 WHEN l.status='New' THEN 1 WHEN l.status='Ringing' THEN 2 WHEN l.status='Interested' THEN 3 WHEN (l.status='Follow-up' OR l.next_followup IS NOT NULL) THEN 4 ELSE 3 END,l.created_at DESC,l.id DESC LIMIT 250";$s=$pdo->prepare($sql);$s->execute($p);$leads=$s->fetchAll();
...
if($_POST['action']==='add_manual_followup'){ $bn=trim($_POST['business_name']??''); $ph=trim($_POST['phone']??''); $fa=trim($_POST['followup_at']??''); $rm=trim($_POST['remark']??''); $src=trim($_POST['lead_source']??'Other'); $mc=trim($_POST['category']??''); if(!in_array($mc,['Photography','Makeup Artist'],true))$mc=in_array($_SESSION['category'],['Photography','Makeup Artist'],true)?$_SESSION['category']:'Photography'; if($bn&&$fa){$existingId=0;if($ph){$qq=$pdo->prepare('SELECT id FROM leads WHERE phone=? AND category=? LIMIT 1');$qq->execute([$ph,$mc]);$existingId=(int)($qq->fetchColumn()?:0);}if($existingId){$pdo->prepare("UPDATE leads SET status='Follow-up',next_followup=?,remark=?,source=?,assigned_to=?,existing_customer=0,updated_at=NOW() WHERE id=?")->execute([$fa,$rm,$src,$me,$existingId]);}else{$x=$pdo->prepare("INSERT INTO leads(business_name,category,phone,source,status,assigned_to,remark,next_followup) VALUES(?,?,?,?, 'Follow-up',?,?,?)");$x->execute([$bn,$mc,$ph?:null,$src,$me,$rm,$fa]);}} header('Location:?view=followups&saved=1'); exit;}
...
<div id="notifyBanner" class="card" style="display:none;margin-top:15px;background:#eff6ff"><b>🔔 Enable notifications</b><div class="muted" style="margin:5px 0 10px">Allow LeadDesk to notify you when your daily leads are ready.</div><button id="enableNotify" class="btn primary">Enable Notifications</button><button id="testNotify" class="btn" style="margin-left:8px">🔔 Test</button></div><div class="nav"><a class="btn <?= $view==='dashboard'?'primary':''?>" href="/">⌂ Dashboard</a><a class="btn <?= $view==='leads'?'primary':''?>" href="?view=leads">📋 My Leads</a><a class="btn <?= $view==='new'?'primary':''?>" href="?view=new&cat=<?=urlencode($cat)?>">🆕 New Leads</a><a class="btn <?= $view==='followups'?'primary':''?>" href="?view=followups">⏰ Follow-ups</a><a class="btn <?= $view==='interested'?'primary':''?>" href="?view=interested">🔥 Interested</a><a class="btn <?= $view==='ringing'?'primary':''?>" href="?view=ringing">📳 Ringing</a><a class="btn <?= $view==='starred'?'primary':''?>" href="?view=starred">⭐ Starred</a><a class="btn <?= $view==='existing'?'primary':''?>" href="?view=existing">👥 Existing</a><?php if($isAdmin):?><a class="btn" href="?view=admin">⚙️ Admin</a><?php endif;?><a class="btn <?= $view==='daily_report'?'primary':''?>" href="?view=daily_report">📷 Daily Report</a><?php if($isAdmin):?><a class="btn <?= $view==='reports'?'primary':''?>" href="?view=reports">📊 Reports</a><?php endif;?></div>
...
<?php
$dashWhere="WHERE l.existing_customer=0";
$dashParams=[];
if(!$isAdmin){$dashWhere.=" AND l.assigned_to=?";$dashParams[]=$me;}
$dashSt=$pdo->prepare("SELECT l.id,l.created_at,l.source FROM leads l $dashWhere ORDER BY l.created_at DESC,l.id DESC LIMIT 5000");
$dashSt->execute($dashParams);
$dashRows=$dashSt->fetchAll();
$dashGroups=[];
foreach($dashRows as $dr){
  $d=date('Y-m-d',strtotime($dr['created_at']));
  if(!isset($dashGroups[$d]))$dashGroups[$d]=[];
  $last=$dashGroups[$d][count($dashGroups[$d])-1]??null;
  $ts=strtotime($dr['created_at']);
  if(!$last || ($last['last_ts']-$ts)>900){
    $dashGroups[$d][]= ['first_ts'=>$ts,'last_ts'=>$ts,'count'=>1,'source'=>$dr['source']?:'Other'];
  }else{
    $dashGroups[$d][count($dashGroups[$d])-1]['count']++;
    $dashGroups[$d][count($dashGroups[$d])-1]['first_ts']=min($dashGroups[$d][count($dashGroups[$d])-1]['first_ts'],$ts);
  }
}
?>
<div class="card" style="margin-top:15px">
<h3>📅 Leads Date-wise</h3>
<p class="muted">Your leads are grouped by the date they were added. Multiple additions on the same date appear as separate batches.</p>
<div class="date-tabs">
<?php foreach($dashGroups as $d=>$batches): $batchNo=count($batches); $i=0; foreach($batches as $b): $i++; ?>
<a class="date-tab" href="?view=leads&date_from=<?=h($d)?>&date_to=<?=h($d)?>&batch_start=<?=h(date('Y-m-d H:i:s',$b['first_ts']))?>&batch_end=<?=h(date('Y-m-d H:i:s',$b['last_ts']))?>">
  <div class="date-tab-title">📅 <?=h(date('jS M Y',strtotime($d)))?><?=($batchNo>1?' ('.$i.')':'')?></div>
  <div class="date-tab-count"><?=$b['count']?> leads</div>
  <div class="muted"><?=h($b['source'])?></div>
</a>
<?php endforeach; endforeach;?>
<?php if(!$dashGroups):?><div class="muted" style="padding:15px 0">No leads added yet.</div><?php endif;?>
</div></div>
<?php elseif($view==='daily_report'):?>
...
<?php else:?><?php if($view==='admin'&&$isAdmin):?><div class="card" style="margin-top:15px"><h3>👥 Team</h3><?php foreach($users as $u){$st=$pdo->query("SELECT COUNT(*) FROM leads WHERE assigned_to=".$u['id']." AND DATE(created_at)=CURDATE()")->fetchColumn();?><p><?=h($u['name'])?> · <?=h($u['category'])?> <b style="float:right;color:#2563eb"><?=$st?> / 50</b></p><?php }?><hr><h4>🔐 Reset Employee Password</h4><form method="post" class="grid"><input type="hidden" name="action" value="reset_password"><select name="user_id" required><option value="">Select employee</option><?php foreach($users as $u):?><option value="<?=$u['id']?>"><?=h($u['name'])?> · <?=h($u['category'])?></option><?php endforeach;?></select><input class="input" type="password" name="new_password" minlength="6" placeholder="New password" required><button class="btn primary" type="submit">Reset Password</button></form><?php if(isset($_GET['password_reset'])):?><p style="color:#15803d;font-weight:700">Password reset successfully.</p><?php endif;?></div><div class="card" style="margin-top:12px"><h3>🔔 Push Notifications</h3><p class="muted">Send an instant push notification to every employee who has enabled LeadDesk notifications.</p><?php if(isset($_GET['push_sent'])):?><p style="color:#15803d;font-weight:700">Sent to <?=intval($_GET['push_sent'])?> devices.</p><?php elseif(isset($_GET['push_error'])):?><p style="color:#dc2626;font-weight:700">Could not send notification.</p><?php endif;?><form method="post" action="push_send.php" class="grid"><input class="input" name="title" maxlength="80" placeholder="Notification title" value="SetMyWed LeadDesk" required><input class="input" name="body" maxlength="240" placeholder="Message" required><input class="input" name="url" value="/" placeholder="Open URL"><button class="btn primary" type="submit">📣 Send to Everyone</button></form></div><div class="card" style="margin-top:12px"><h3>📥 Import & Distribute Leads</h3><p class="muted">Paste raw text or CSV data. LeadDesk will detect business name, phone, city, Instagram and website, then distribute the leads equally among selected employees.</p><a class="btn primary" href="import_leads.php">Open Lead Importer →</a></div><div class="card" style="margin-top:12px"><h3>📸 Instagram Leads</h3><p class="muted">Search public web results like “Bridal Makeup Artist + Delhi” and automatically collect Instagram profiles, remove duplicates, and distribute new leads.</p><a class="btn primary" href="instagram_search.php">Open Instagram Importer →</a></div><div class="card" style="margin-top:12px"><h3>➕ Add Lead</h3><form method="post" class="grid" style="margin-top:10px"><input type="hidden" name="action" value="add_lead"><input class="input" name="business_name" placeholder="Business / vendor name" required><input class="input" name="city" placeholder="City"><input class="input" name="phone" placeholder="Phone"><input class="input" name="instagram" placeholder="Instagram URL"><input class="input" name="website" placeholder="Website URL"><select name="category" required><option value="">Category</option><option>Photography</option><option>Makeup Artist</option></select><select name="assigned_to" required><option value="">Assign employee</option><?php foreach($users as $u):?><option value="<?=$u['id']?>"><?=h($u['name'])?> · <?=h($u['category'])?></option><?php endforeach;?></select><input class="input" type="number" name="lead_score" value="50" min="0" max="100"><div><button class="btn primary" type="submit">Add Lead</button></div></form></div>
...
<form method="post" class="grid"><input type="hidden" name="action" value="add_manual_followup"><input class="input" name="business_name" placeholder="Vendor / business name" required><input class="input" name="phone" placeholder="Phone number"><select name="category" required><option value="">Category</option><option value="Photography" <?=($_SESSION["category"]==="Photography"?"selected":"")?>>📷 Photography</option><option value="Makeup Artist" <?=($_SESSION["category"]==="Makeup Artist"?"selected":"")?>>💄 Makeup Artist</option></select><input class="input" type="datetime-local" name="followup_at" required><input class="input" name="lead_source" placeholder="Lead source (WhatsApp, Referral, Instagram...)"><textarea name="remark" rows="2" placeholder="Remark"></textarea><div><button class="btn primary">➕ Add Follow-up</button></div></form></div>
...
async function enableLeadDeskNotifications(){
  ...
  try{
    ...
    const cfg=await fetch("push_config.php",{credentials:"same-origin"}).then(r=>r.json());
    if(!cfg.publicKey)throw new Error("Push service unavailable");
    const key=Uint8Array.from(atob(cfg.publicKey.replace(/-/g,'+').replace(/_/g,'/')+'='.repeat((4-cfg.publicKey.length%4)%4)),c=>c.charCodeAt(0));
    ...
    alert("Push notifications enabled. You will receive LeadDesk notifications from 10 AM to 6 PM, Monday to Friday.");
...
// Scheduled reminders are now delivered server-side through Web Push.\nini_set('session.gc_maxlifetime','2592000');
session_set_cookie_params(['lifetime'=>2592000,'path'=>'/','secure'=>true,'httponly'=>true,'samesite'=>'Lax']);
session_start();
$c=require __DIR__.'/config.php';
try{$pdo=new PDO("mysql:host={$c['db']['host']};port={$c['db']['port']};dbname={$c['db']['name']};charset=utf8mb4",$c['db']['user'],$c['db']['pass'],[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC]);}catch(Throwable $e){http_response_code(500);die('Database connection failed.');}
